...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ===== Load Entries from Firestore (supports two patterns) =====
  async function loadEntries() {
    ...
    // make globally accessible (your other code expects window.entries)
    window.entries = entriesObj;
    renderExportList();
    await updateStatsAndSave();
  }
  // expose so other modules can call it if needed
  ...
  // ===== Upload & Crop Image (kept mostly intact) =====
  ...
  // ImgBB key already in your code — keep it safe (don't expose in public repos).
  ...
  async function updateStatsAndSave() {
    ...
    // save back to Firestore so it's available elsewhere
    try {
      const userDocRef = doc(db, "users", window.currentUser.uid);
